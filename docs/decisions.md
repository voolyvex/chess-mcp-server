...
| 5 | Result shape | Pure engine evidence + WDL. **No narration layer** — the assistant writes the prose, every number is engine-produced. |
...
| 12 | Repo layout | **Public repo, `chess-mcp-server`**, with the GitHub remote created before the first commit so CI runs from it. The engine ships in-tree as `engine/`, with an engine-only `docker-compose.yml` — no database, per #5. |
...
A → #12 · B → #13 · C → #14, #15 · D → #9 · F → #16, #17 · G → #18, #19
...
## The sign convention, confirmed empirically

**UCI `score cp` is reported from the side-to-move's perspective.** Confirmed against
Stockfish 18 WASM (the build in `node_modules`), depth 14, one board evaluated twice with
...
Pass row 2 through unconverted and you report "White is winning" while White is down a
queen. This is why #14 ships `evaluation_cp`, `raw_score_cp` and `side_to_move` together:
the conversion is auditable rather than trusted.

**Prior art — two valid conventions.**
...
`fen`, raw `score` and a normalized `evaluation` side by side — but **no depth and no
engine version**, the gap #5 closes.
[`dvip1/mcp-chess`](https://glama.ai/mcp/servers/dvip1/chess_mcp) stays side-to-move
relative and documents it. Either is defensible. What is not is leaving the convention
unstated in the code, so nothing forces the parse end and the render end to agree.
